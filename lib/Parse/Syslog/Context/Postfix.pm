package Parse::Syslog::Context::Postfix;
...
information, IP addresses, and email addresses.
    ...
    component => Postfix daemon issuing the message, e.g.: smtpd, anvil
    program   => Set to 'postfix' for simplicity
    proto     => ESMTP, SMTP, LMTP, etc.
...
sub process {
    ...
    return unless $log->{program_name} eq 'postfix';

    my $str = $log->{content};

    $ctx->{_tags}{mail} = 1;
    if( my @connection = ($str =~ /^((?:dis)?connect) from ([^\[]+)\[([^\]]+)\]/) ) {
        @{ $ctx }{qw(action src src_ip)} = @connection;
    }
    elsif( my @details = ($str =~ /^([A-F0-9]{8,16}): (.*)$/) ) {
        $ctx->{rec_id} = $details[0];
        foreach my $kv ( split /, /, $details[1] ) {
            if( my ($k,$v) = ($kv =~ /(\w+)=<?([^>]+)>?/) ) {
                $ctx->{$MAP{$k}} = $v if exists $MAP{$k};
            }
        }
    }
    else {
    ...
    if ( $log->{SDATA} ) {
        foreach my $k (sort keys %MAP) {
            next unless exists $log->{SDATA}{$k};
            $ctx->{$MAP{$k}} = $log->{SDATA}{$k};
        }
    }
}
