package Parse::Syslog::Context::IPTables;
...
sub process {
    ...
    return unless $log->{program_name} eq 'kernel';

    my ($content,$reference) = split /\[/, $log->{content};
    ...
    if( keys %data ) {
        ...
        $data{provider} = 'iptables';
        $ctx->{_tags}{$_} = 1 for qw(security firewall kernel);
    }
}
